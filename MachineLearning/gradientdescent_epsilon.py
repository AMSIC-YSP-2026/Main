...
cost_history = []   # <-- new: track MSE at every iteration
...
while iteration < MAX_ITERATIONS:
    ...
    cost_history.append(mse)   # <-- new: record this iteration's cost
    ...
    if iteration % 100 == 0:
        print(f"Iteration {iteration}: MSE = {mse:.6f}, theta = {theta}, |grad| = {np.linalg.norm(gradient):.8f}")
...
np.save(COST_HISTORY_PATH, np.array(cost_history))   # <-- new: save cost history
